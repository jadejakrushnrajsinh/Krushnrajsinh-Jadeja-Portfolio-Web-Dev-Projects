...
class AdminDashboard {
  ...
  setupEventListeners() {
    ...
    if (addSocialLinkBtn) {
      addSocialLinkBtn.addEventListener("click", () => this.addSocialLinkField());
    }
  ...
  handleKeyboardShortcuts(e) {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === "k") {
      ...
      const searchInput = document.querySelector(`[data-section="${currentSection}"]`);
      if (searchInput) {
        searchInput.focus();
      }
    }
  ...
  updateStats(stats) {
    ...
    document.getElementById("messageCount").textContent = stats.unreadMessages || 0;
    document.getElementById("messageCount").style.display = stats.unreadMessages > 0 ? "inline" : "none";
  }
  ...
  renderActivity(activity) {
    ...
    if (!activity || activity.length === 0) {
      container.innerHTML = '<div class="empty-state"><i class="fas fa-chart-line"></i><h3>No recent activity</h3></div>';
      return;
    }
  ...
  renderFeaturedProjects(projects) {
    ...
    if (!projects || projects.length === 0) {
      container.innerHTML = '<div class="empty-state"><i class="fas fa-star"></i><h3>No featured projects</h3></div>';
      return;
    }

    container.innerHTML = projects.map((project) => `
      <div class="featured-item animate-slide-right">
        <img src="${project.imageUrl || "/placeholder.jpg"}" alt="${project.title}" class="featured-image">
  ...
  applyProjectFilters(projects) {
    ...
    return projects.filter(project => {
      ...
      const matchesSearch = !search ||
        ...
        (project.technologies && project.technologies.some(tech => tech.toLowerCase().includes(search)));
  ...
  renderProjects(projects) {
    ...
    if (!projects || projects.length === 0) {
      container.innerHTML = '<div class="empty-state"><i class="fas fa-folder-open"></i><h3>No projects found</h3><p>Create your first project to get started.</p></div>';
      return;
    }

    container.innerHTML = projects.map((project, index) => `
      <div class="project-card animate-bounce" style="animation-delay: ${index * 0.1}s" data-id="${project._id}">
        ...
        <div class="project-content">
          ...
          <div class="project-actions">
            <button class="btn btn-sm btn-secondary" onclick="dashboard.editProject('${project._id}')">
              <i class="fas fa-edit"></i> Edit
            </button>
            <button class="btn btn-sm btn-danger" onclick="dashboard.deleteProject('${project._id}')">
  ...
  renderMessages(messages) {
    ...
    if (!messages || messages.length === 0) {
      container.innerHTML = '<div class="empty-state"><i class="fas fa-envelope"></i><h3>No messages found</h3></div>';
      return;
    }

    container.innerHTML = messages.map((message, index) => `
      <div class="message-item animate-slide-left ${message.status === "New" ? "unread" : ""}"
        ...
        <div class="message-preview">${message.message.substring(0, 150)}...</div>
        <div class="message-actions">
          <button class="btn btn-sm btn-secondary" onclick="dashboard.viewMessage('${message._id}')">
            <i class="fas fa-eye"></i> View
          </button>
          <button class="btn btn-sm btn-success" onclick="dashboard.markAsRead('${message._id}')">
            <i class="fas fa-check"></i> Mark Read
          </button>
          <button class="btn btn-sm btn-danger" onclick="dashboard.deleteMessage('${message._id}')">
            <i class="fas fa-trash"></i> Delete
          </button>
        </div>
      </div>
  ...
  renderTasks(tasks) {
    ...
    if (!tasks || tasks.length === 0) {
      container.innerHTML = '<div class="empty-state"><i class="fas fa-tasks"></i><h3>No tasks found</h3><p>Create your first task to get started.</p></div>';
      return;
    }

    container.innerHTML = tasks.map((task, index) => `
      <div class="task-item animate-slide-right" style="animation-delay: ${index * 0.05}s" data-id="${task._id}">
        ...
        <div class="task-actions">
          <button class="btn btn-sm btn-secondary" onclick="dashboard.editTask('${task._id}')">
            <i class="fas fa-edit"></i>
          </button>
          <button class="btn btn-sm btn-danger" onclick="dashboard.deleteTask('${task._id}')">
  ...
  addSelectionCheckboxes(sectionId) {
    const container = document.getElementById(`${sectionId}List`) || document.getElementById(`${sectionId}Grid`);
    if (!container) return;

    const items = container.querySelectorAll(`.${sectionId === 'projects' ? 'project-card' : sectionId === 'messages' ? 'message-item' : 'task-item'}`);
    ...
    items.forEach(item => {
      if (!item.querySelector('.selection-checkbox')) {
        ...
        checkbox.addEventListener('change', () => this.updateSelection(sectionId));
  ...
  updateSelection(sectionId) {
    const checkboxes = document.querySelectorAll(`#${sectionId}Section .selection-checkbox:checked`);
  ...
  async bulkDelete(sectionId) {
    ...
    try {
      const deletePromises = Array.from(this.selectedItems).map(id => {
        let endpoint;
        switch (sectionId) {
          case 'messages': endpoint = `/api/contact/${id}`; break;
      ...
      this.showToast(`Successfully deleted ${this.selectedItems.size} ${sectionId}`, 'success');
      this.selectedItems.clear();
  ...
  async bulkMarkAsRead() {
    ...
    try {
      ...
      this.showToast(`Marked ${this.selectedItems.size} messages as read`, 'success');
      this.selectedItems.clear();
      this.loadMessages();
    } catch (error) {
  ...
  async loadAnalytics() {
    try {
      ...
      const analytics = await this.apiCall(`/api/analytics/dashboard?range=${range}`);
  ...
  renderAnalytics(data) {
    ...
    if (popularProjectsEl) {
      popularProjectsEl.innerHTML = data.popularProjects?.map(project =>
    ...
    if (visitorLocationsEl) {
      visitorLocationsEl.innerHTML = data.locations?.map(location =>
        `<div class="analytics-item">
  ...
  async loadSettings() {
    try {
      ...
      document.getElementById("footerCopyright").value = settings.footer?.copyright || "";
      document.getElementById("footerCustomText").value = settings.footer?.customText || "";
